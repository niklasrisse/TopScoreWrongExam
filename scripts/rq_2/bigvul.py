...
data = pd.read_csv('./original_datasets/BigVul/MSR_data_cleaned.csv')

train_index, test_index = train_test_split(range(len(data)), test_size=0.2, random_state=fixed_seed)
...
def encodeDataframe(df, tokenizer):
    ...
    for sentence in df.func_before.tolist():
